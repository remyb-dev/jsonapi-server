...
swaggerValidator.assert = (params, statusCode, json) => {
  ...
  swaggerValidator._validateRequest(
    params.method.toLowerCase(),
    urlObj.pathname,
    JSON.parse(params.body || 'null')
  )
  swaggerValidator._validatePayload(
    params.method.toLowerCase(),
    urlObj.pathname,
    statusCode,
    JSON.parse(json)
...
swaggerValidator._validateRequest = (method, path, body) => {
  ...
  const bodySchema = model.parameters
  ...
  return swaggerValidator._validateModel(
    bodySchema.schema,
    body,
    `${method}@${path}`,
    'request',
...
swaggerValidator._validatePayload = (method, path, httpCode, payload) => {
  ...
  if (!schema)
    throw new Error(`Unknown payload for ${method}, ${path}, ${httpCode}`)
...
swaggerValidator._getModel = (method, path) => {
  ...
  let match = Object.keys(swaggerDoc.paths)
    .filter(somePath => {
  ...
  if (!match) {
    if (path.indexOf('foobar') !== -1) {
      return {
        responses: { default: { schema: { $ref: '#/definitions/error' } } },
      }
    }
  ...
  if (!match) {
    throw new Error(
      `Swagger Validation: No matching path for ${method} ${path}`
    )
  }
  return match
}

swaggerValidator._validateModel = (
  model,
  payload,
  urlPath,
  validationPath,
  ...
  if (required && !payload) {
    throw new Error(
      `Swagger Validation: ${urlPath} Expected required value at ${validationPath}`
    )
  }
  if (!payload) return
...
swaggerValidator._validateArray = (model, payload, urlPath, validationPath) => {
  if (!(payload instanceof Array)) {
    throw new Error(
      `Swagger Validation: ${urlPath} Expected Array at ${validationPath}`
    )
  }
  payload.forEach((i, j) => {
    swaggerValidator._validateModel(
...
swaggerValidator._validateObject = (
  model,
  payload,
  urlPath,
  validationPath
  ...
  for (const j in payload) {
    if (!model.properties[j]) {
      throw new Error(
        `Swagger Validation: ${urlPath} Found unexpected property at ${validationPath}.${j}`
      )
    }
  }
}
...
swaggerValidator._validateOther = (model, payload, urlPath, validationPath) => {
  if (model.type === 'string') {
    if (typeof payload !== 'string') {
      throw new Error(
        `Swagger Validation: ${urlPath} Expected string at ${validationPath}, got ${typeof payload}`
      )
    }
  } else if (model.type === 'number') {
    if (typeof payload !== 'number') {
      throw new Error(
        `Swagger Validation: ${urlPath} Expected number at ${validationPath}, got ${typeof payload}`
      )
    }
  } else if (model.type === 'boolean') {
    if (typeof payload !== 'boolean') {
      throw new Error(
        `Swagger Validation: ${urlPath} Expected boolean at ${validationPath}, got ${typeof payload}`
      )
    }
  } else {
    throw new Error(
