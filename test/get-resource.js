  ...
  describe('Searching for resources', () => {
      ...
      helpers.request(
        ...
        (err, res, json) => {
          ...
          assert.deepEqual(
            json.included,
            [],
            'Response should have no included resources'
          )
          assert.equal(
            json.data.length,
            4,
            'Response should contain exactly 4 resources'
          )
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            for (let i = 1; i < json.data.length; i++) {
              current = json.data[i]
              assert.ok(
                previous.attributes.title < current.attributes.title,
    ...
    describe('applying filter', () => {
      ...
      it('unknown multiple attribute should error', done => {
        const url =
          'http://localhost:16006/rest/articles?filter[foo]=bar&filter[foo]=baz'
        helpers.request(
          {
            method: 'GET',
            url,
      ...
      it('less than for numbers', done => {
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            assert.deepEqual(
              titles,
              ['Linux Rocks', 'NodeJS Best Practices'],
              'expected matching resources'
            )
      ...
      it('greater than for strings', done => {
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            assert.deepEqual(
              titles,
              ['NodeJS Best Practices', 'Tea for Beginners'],
              'expected matching resources'
            )
      ...
      it('case insensitive for non-string types', done => {
        const url =
          'http://localhost:16006/rest/articles?filter[created]=~2016-01-01'
        helpers.request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.data.length,
              0,
              "didn't expect matching resources"
            )
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.data.length,
              2,
              'Should only give the 2x requested resources'
            )
      ...
      it('allows for multiple filter values to be combined in a comma-separated list', done => {
        const url =
          'http://localhost:16006/rest/articles?filter[tags]=6ec62f6d-9f82-40c5-b4f4-279ed1765492,7541a4de-4986-4597-81b9-cf31b6762486,2a3bdea4-a889-480d-b886-104498c86f69'
        helpers.request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.data.length,
              3,
              'Should only give the 3x requested resources'
            )
      ...
      it('allows for a compound of comma-separated list filters', done => {
        const url =
          'http://localhost:16006/rest/articles?filter[tags]=6ec62f6d-9f82-40c5-b4f4-279ed1765492,7541a4de-4986-4597-81b9-cf31b6762486,2a3bdea4-a889-480d-b886-104498c86f69&filter[id]=de305d54-75b4-431b-adb2-eb6b9e546014,1be0913c-3c25-4261-98f1-e41174025ed5'
        helpers.request(
          {
            method: 'GET',
            url,
      ...
      it('allows deep filtering', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author&filter[author]=d850ea75-4427-4f81-8595-039990aeede5&filter[author][firstname]=Mark'
        helpers.request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.data.length,
              1,
              'Should give the one matching resource'
            )
            assert.equal(
              json.included.length,
              1,
              'Should give the one matching include'
            )
    ...
    describe('applying fields', () => {
      ...
      it('just title', done => {
        const url =
          'http://localhost:16006/rest/articles?fields[articles]=title'
        request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            json.data.forEach(resource => {
              ...
              assert.deepEqual(
                keys,
                ['title'],
                'should only have the title attribute'
              )
            })
    ...
    describe('applying includes', () => {
      ...
      it('include author', done => {
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            const people = json.included.filter(
              resource => resource.type === 'people'
            )
            assert.equal(
              people.length,
              4,
              'Should be 4 included people resources'
            )
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.included.length,
              7,
              'Should be 7 included resources'
            )
            ...
            const photos = json.included.filter(
              resource => resource.type === 'photos'
            )
            assert.equal(
              photos.length,
              3,
              'Should be 3 included photos resources'
            )
        ...
        helpers.request(
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.included.length,
              8,
              'Should be 8 included resources'
            )
            ...
            assert.equal(
              photos.length,
              4,
              'Should be 4 included photos resources'
            )
      ...
      it('include author with filter and people fields', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author&filter[title]=Linux+Rocks&fields[people]=email'
        request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.data[0].attributes.created,
              '2015-11-11',
              'Article should have created attribute'
            )

            assert.equal(
              json.included.length,
              1,
              'Should be 1 included resource'
            )
            const people = json.included.filter(
              resource => resource.type === 'people'
            )
            assert.equal(
              people.length,
              1,
              'Should be 1 included people resource'
            )
            assert.equal(
              people[0].attributes.email,
              'mark.fermor@example.com',
              'Included people should have email attribute'
            )
      ...
      it('include author with filter and article fields', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author&filter[title]=Linux+Rocks&fields[articles]=created'
        request(
          {
            method: 'GET',
            url,
      ...
      it('include author with filter and people+article fields', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author&filter[title]=Linux+Rocks&fields[people]=email&fields[articles]=created'
        request(
          {
            method: 'GET',
            url,
      ...
      it('include author.photos with filter', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author.photos&filter[author][firstname]=Mark'
        helpers.request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(
              json.included.length,
              2,
              'Should be 2 included resources'
            )
            ...
            assert.equal(
              photos.length,
              1,
              'Should be 1 included photos resource'
            )
      ...
      it('include author.photos with multiple filters', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author.photos&filter[author]=ad3aa89e-9c5b-4ac9-a652-6670f9f27587&filter[author]=cc5cca2e-0dd8-4b95-8cfc-a11230e73116'
        helpers.request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(
              people.length,
              2,
              'Should be 2 included people resource'
            )
            ...
            assert.equal(
              photos.length,
              3,
              'Should be 2 included photos resource'
            )
      ...
      it('include author.photos with multiple filters comma delineated', done => {
        const url =
          'http://localhost:16006/rest/articles?include=author.photos&filter[author][firstname]=Mark,Oli'
        helpers.request(
          {
            method: 'GET',
            url,
    ...
    describe('by foreign key', () => {
      it('should find resources by a relation', done => {
        const url =
          'http://localhost:16006/rest/articles/?filter[photos]=aab14844-97e7-401c-98c8-0bd5ec922d93'
        helpers.request(
          {
            method: 'GET',
            url,
      ...
      it('should find resources by many relations', done => {
        const url =
          'http://localhost:16006/rest/articles/?filter[photos]=aab14844-97e7-401c-98c8-0bd5ec922d93&filter[photos]=4a8acd65-78bb-4020-b9eb-2d058a86a2a0'
        helpers.request(
          {
            method: 'GET',
            url,
      ...
      it('should error with incorrectly named relations', done => {
        const url =
          'http://localhost:16006/rest/articles/?filter[photo]=aab14844-97e7-401c-98c8-0bd5ec922d93'
        helpers.request(
          {
            method: 'GET',
            url,
      ...
      it('should error when querying the foreign end of a relationship', done => {
        const url =
          'http://localhost:16006/rest/comments/?filter[article]=aab14844-97e7-401c-98c8-0bd5ec922d93'
        helpers.request(
          {
            method: 'GET',
            url,
          ...
          (err, res, json) => {
            ...
            assert.equal(error.title, 'Invalid filter')
            assert(
              error.detail.match('is a foreign reference and does not exist on')
            )
            done()
          }
        )
