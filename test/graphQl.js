...
describe('Testing jsonapi-server graphql', () => {
  describe('read operations', () => {
    it('filter with primary join and filter', () =>
      client
        .query(
          `
      {
    ...
    it('filter with foreign join and filter', () =>
      client
        .query(
          `
      {
  ...
  describe('write operations', () => {
    ...
    `
        ...
        .then(result => {
          ...
          assert.equal(
            result.createTags.parent.id,
            '7541a4de-4986-4597-81b9-cf31b6762486'
          )
          assert.equal(result.createTags.parent.name, 'live')
          tagId = result.createTags.id
        }))

    it('update the new tag', () =>
      client
        .mutate(
          `
      {
