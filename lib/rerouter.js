...
rerouter.route = (newRequest, callback) => {
  ...
  const req = {
    ...
    params: rerouter._mergeParams(
      url.parse(newRequest.uri.split('?')[1] || {}, qsOpts),
      newRequest.params
    ),
  }
  ...
  validRoutes[route](
    req,
    res,
    _.omit(newRequest.originalRequest, ['params', 'route'])
  )
}
...
rerouter._pickFirstMatchingRoute = (validRoutes, path) =>
  Object.keys(validRoutes)
    .filter(someRoute => {
