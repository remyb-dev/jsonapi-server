...
MemoryStore.prototype.search = function (request, callback) {
  ...
  if (request.params.page) {
    results = results.slice(
      ...
      request.params.page.offset + request.params.page.limit
    )
  }
...
MemoryStore.prototype.find = (request, callback) => {
  ...
  const theResource = resources[request.params.type]
    .filter(anyResource => anyResource.id === request.params.id)
    .pop()
...
MemoryStore.prototype.create = (request, newResource, callback) => {
  ...
  const index = MemoryStore._indexOf(
    resources[request.params.type],
    newResource
  )
  if (index !== -1) {
    return callback({
