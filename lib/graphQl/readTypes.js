...
readTypes.generate = allResourceConfig => {
  ...
  Object.keys(allResourceConfig).forEach(resource => {
    allReadTypes[resource] = readTypes.createReadType(
      allResourceConfig[resource],
      allReadTypes,
      allUnionTypes
    )
  })
...
readTypes.createReadType = (resourceConfig, otherTypes, allUnionTypes) => {
    ...
    fields() {
      ...
      Object.keys(resourceConfig.attributes).forEach(attribute => {
        ...
        if (joiScheme._flags._settings) {
          const otherResource =
            joiScheme._flags._settings.__one || joiScheme._flags._settings.__many
  ...
  Object.keys(resourceConfig.attributes).forEach(attribute => {
    const joiScheme = resourceConfig.attributes[attribute]
    if (!joiScheme._flags._settings) return
    const otherResource =
      joiScheme._flags._settings.__one || joiScheme._flags._settings.__many
