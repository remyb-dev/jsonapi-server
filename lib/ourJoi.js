'use strict'
const ourJoi = (module.exports = {})

const Joi = require('joi')

ourJoi._joiBase = resourceName => {
  const relationType = Joi.object().keys({
    id: Joi.string().required(),
    type: Joi.any().required().valid(resourceName),
    meta: Joi.object().optional(),
  })
  return relationType
}

// Joi.one = function () {
//   const resources = Array.prototype.slice.call(arguments)
//   resources.forEach(resource => {
//     if (typeof resource !== 'string')
//       throw new Error(
//         'Expected a string when defining a primary relation via .one()'
//       )
//   })
//   const obj = Joi.alternatives().try(
//     ...[
//       Joi.any().valid(null), // null
//     ].concat(resources.map(ourJoi._joiBase))
//   )
//   obj._flags._settings = {
//     __one: resources,
//   }
//   return obj
// }

// Joi.many = function () {
//   const resources = Array.prototype.slice.call(arguments)
//   resources.forEach(resource => {
//     if (typeof resource !== 'string')
//       throw new Error(
//         'Expected a string when defining a primary relation via .many()'
//       )
//   })
//   const obj = Joi.array().items(...resources.map(ourJoi._joiBase))
//   // console.trace('_flags._settings')
//   obj._flags._settings = {
//     __many: resources,
//   }
//   return obj
// }
Joi._validateForeignRelation = config => {
  if (!config.as)
    throw new Error("Missing 'as' property when defining a foreign relation")
  if (!config.resource)
    throw new Error(
      "Missing 'resource' property when defining a foreign relation"
    )
}
// Joi.belongsToOne = config => {
//   Joi._validateForeignRelation(config)
//   const obj = Joi.alternatives().try(
//     Joi.any().valid(null), // null
//     ourJoi._joiBase(config.resource)
//   )
//   obj._flags._settings = {
//     __one: [config.resource],
//     __as: config.as,
//   }
//   return obj
// }
// Joi.belongsToMany = config => {
//   Joi._validateForeignRelation(config)
//   const obj = Joi.array().items(ourJoi._joiBase(config.resource))
//   obj._flags._settings = {
//     __many: [config.resource],
//     __as: config.as,
//   }
//   return obj
// }
ourJoi.Joi = Joi.extend(
  joi => {
    return {
      type: 'belongsToOne',
      rules: {
        config: {
          alias: 'config',
          method(config) {
            return this.$_setFlag('_settings', {
              __many: [config.resource],
              __as: config.as,
            })
          },
        },
      },
      validate(value, helpers, args) {
        const validation = Joi.alternatives()
          .try(
            Joi.any().valid(null), // null
            Joi.object().keys({
              id: Joi.string().required(),
              type: Joi.any()
                .required()
                .valid(...helpers.schema.$_getFlag('_settings').__many),
              meta: Joi.object().optional(),
            })
          )
          .validate(value)

        return { value: validation.value, errors: validation.error }
      },
    }
  },
  joi => {
    return {
      type: 'belongsToMany',
      rules: {
        config: {
          alias: 'config',
          method(config) {
            return this.$_setFlag('_settings', {
              __many: [config.resource],
              __as: config.as,
            })
          },
        },
      },
      validate(value, helpers, args) {
        const validation = Joi.array()
          .items(
            Joi.object().keys({
              id: Joi.string().required(),
              type: Joi.any()
                .required()
                .valid(...helpers.schema.$_getFlag('_settings').__one),
              meta: Joi.object().optional(),
            })
          )
          .validate(value)

        return { value: validation.value, errors: validation.error }
      },
    }
  },
  joi => {
    return {
      type: 'one',

      rules: {
        resources: {
          alias: 'resources',
          method(...resources) {
            return this.$_setFlag('_settings', { __one: resources })
          },
        },
      },
      validate(value, helpers, args) {
        const validation = Joi.alternatives()
          .try(
            ...[
              Joi.any().valid(null), // null
            ].concat(
              Joi.object().keys({
                id: Joi.string().required(),
                type: Joi.any()
                  .required()
                  .valid(...helpers.schema.$_getFlag('_settings').__one),
                meta: Joi.object().optional(),
              })
            )
          )
          .validate(value)

        return { value: validation.value, errors: validation.error }
      },
    }
  },
  joi => {
    return {
      type: 'many',
      rules: {
        resources: {
          alias: 'resources',
          method(...resources) {
            resources.forEach(resource => {
              if (typeof resource !== 'string')
                throw new Error(
                  'Expected a string when defining a primary relation via .many()'
                )
            })
            return this.$_setFlag('_settings', { __many: resources })
          },
        },
      },
      validate(value, helpers, args) {
        const validation = Joi.array()
          .items(
            Joi.object().keys({
              id: Joi.string().required(),
              type: Joi.any()
                .required()
                .valid(...helpers.schema.$_getFlag('_settings').__many),
              meta: Joi.object().optional(),
            })
          )
          .validate(value)

        return { value: validation.value, errors: validation.error }
      },
    }
  }
)
