...
swaggerPaths._addPathDefinition = (paths, resourceConfig) => {
  ...
  Object.keys(resourceConfig.attributes)
    .filter(relationName => {
      let relation = resourceConfig.attributes[relationName]
      relation = relation._flags._settings
      if (!relation || relation.__as) return false
      relation = (relation.__many || relation.__one)[0]
      return (
        jsonApi._resources[relation] &&
        jsonApi._resources[relation].handlers.find
      )
    })
    .forEach(relationName => {
      let relation = resourceConfig.attributes[relationName]
      relation = (relation._flags._settings.__one || relation._flags._settings.__many)[0]

      swaggerPaths._addDeepPaths(
        paths,
        resourceName,
        resourceConfig,
        relationName,
...
swaggerPaths._addDeepPaths = (
  ...
  const relationType = resourceConfig.attributes[relationName]._flags._settings.__many
    ? 'many'
    : 'one'
  ...
  if (resourceConfig.handlers.find) {
    paths[`/${resourceName}/{id}/${relationName}`] = {
      get: swaggerPaths._getPathOperationObject({
        handler: 'find',
        description: `Get the ${relationName} instance${
          relationType === 'many' ? 's' : ''
        } of a specific instance of ${resourceName}`,
...
swaggerPaths._getPathOperationObject = options => {
  ...
  if (
    ...
    pathDefinition.parameters = pathDefinition.parameters.concat(
