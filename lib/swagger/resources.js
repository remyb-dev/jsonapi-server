...
swaggerPaths.getResourceDefinitions = () => {
  ...
  for (const resource in jsonApi._resources) {
    resourceDefinitions[resource] = swaggerPaths._getResourceDefinition(
      jsonApi._resources[resource]
    )
  }
...
swaggerPaths._getResourceDefinition = resourceConfig => {
  ...
  const relationshipsShortcut =
    resourceDefinition.properties.relationships.properties

  const attributes = resourceConfig.attributes
  for (const attribute in attributes) {
    if (attribute === 'id' || attribute === 'type' || attribute === 'meta')
      continue
    ...
    if (!joiScheme._flags._settings) {
      swaggerScheme.type = joiScheme.type
      ...
      if (joiScheme._flags._settings.as) continue
      ...
      if (joiScheme._flags._settings.__many) {
