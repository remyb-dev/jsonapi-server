...
handlerEnforcer._wrapHandler = (handlers, operation, outCount) => {
  ...
  return function () {
    ...
    argsIn.push(function () {
      ...
      debug.handler[operation](
        JSON.stringify(requestParams),
        JSON.stringify(argsOut)
      )
...
handlerEnforcer._search = handlers =>
  handlerEnforcer._wrapHandler(handlers, 'search', 3)

handlerEnforcer._find = handlers =>
  handlerEnforcer._wrapHandler(handlers, 'find', 2)

handlerEnforcer._create = handlers =>
  handlerEnforcer._wrapHandler(handlers, 'create', 2)

handlerEnforcer._update = handlers =>
  handlerEnforcer._wrapHandler(handlers, 'update', 2)

handlerEnforcer._delete = handlers =>
  handlerEnforcer._wrapHandler(handlers, 'delete', 1)
