...
includePP.action = (request, response, callback) => {
  ...
  includePP._arrayToTree(request, includes, filters, (attErr, includeTree) => {
    ...
    includePP._fillIncludeTree(includeTree, request, fiErr => {
      ...
      response.included = _.uniqBy(
        response.included,
        someItem => `${someItem.type}~~${someItem.id}`
      )
...
includePP._arrayToTree = (request, includes, filters, callback) => {
  ...
  // console.log(request.resourceConfig)
  ...
  const iterate = (text, node, filter) => {
    ...
    // console.log(node._resourceConfig)
    if (!filter) filter = {}

    let resourceAttribute = node._resourceConfig
      .map(resourceConfig => {
        return resourceConfig.attributes[first]
      })
      .filter(a => a)
      .pop()
    console.log(first, resourceAttribute)
    ...
    resourceAttribute =
      resourceAttribute._settings &&
      (resourceAttribute._settings.__one || resourceAttribute._settings.__many)
    if (!resourceAttribute) {
      // console.log(node._resourceConfig)
      return validationErrors.push({
        status: '403',
    ...
    if (!node[first]) {
      ...
      if (!(Array.isArray(filter) && filter.length === 0)) {
        for (const i in filter) {
          if (!(typeof filter[i] === 'string' || Array.isArray(filter[i])))
            continue
...
includePP._fillIncludeTree = (includeTree, request, callback) => {
  ...
  ****/
  ...
  includeTree._dataItems.forEach(dataItem => {
    if (!dataItem) return []
    return Object.keys(dataItem.relationships || {})
      .filter(keyName => keyName[0] !== '_' && includes.indexOf(keyName) !== -1)
      .forEach(relation => {
    ...
    (related, done) => {
      ...
      rerouter.route(
        ...
        (err, json) => {
          ...
          includeTree[parts[2]]._dataItems =
            includeTree[parts[2]]._dataItems.concat(data)
          return done()
        }
      )
    },
