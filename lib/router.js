...
router.applyMiddleware = () => {
  ...
  app.use((req, res, next) => {
    ...
    if (req.headers['content-type']) {
      ...
      if (
        req.headers['content-type'].match(/^application\/vnd\.api\+json;.+$/)
      ) {
        return res
          .status(415)
          .end(
            `HTTP 415 Unsupported Media Type - [${req.headers['content-type']}]`
          )
      }
    ...
    if (req.headers.accept) {
      ...
      matchingTypes = matchingTypes.filter(
        (
          mediaType // Accept application/*, */vnd.api+json, */* and the correct JSON:API type.
        ) =>
          mediaType.match(/^(\*|application)\/(\*|json|vnd\.api\+json)$/) ||
          mediaType.match(/\*\/\*/)
      )
...
router.bindRoute = (config, callback) => {
  ...
  const routeHandler = (req, res, extras) => {
    ...
    router.authenticate(request, res, () =>
      callback(request, resourceConfig, res)
    )
  }
