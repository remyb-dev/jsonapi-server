...
responseHelper._enforceSchemaOnObject = (item, schema, callback) => {
  ...
  const { value, error } = Joi.object(schema).validate(item)
  if (error) {
    debug.validationError(error.message, JSON.stringify(item))
    const res = {
      status: '500',
      code: 'EINVALIDITEM',
  ...
  const dataItem = responseHelper._generateDataItem(value, schema)
...
responseHelper._generateDataItem = (item, schema) => {
  const isSpecialProperty = value => {
    ...
    if (value._flags._settings) return true
    return false
  }
  const linkProperties = Object.keys(schema).filter(someProperty =>
    isSpecialProperty(schema[someProperty])
  )
  ...
  const result = {
    ...
    relationships: responseHelper._generateRelationships(
      item,
      schema,
      linkProperties
    ),
...
responseHelper._generateRelationships = (item, schema, linkProperties) => {
  ...
  linkProperties.forEach(linkProperty => {
    links[linkProperty] = responseHelper._generateLink(
      item,
      schema[linkProperty],
      linkProperty
    )
...
responseHelper._generateLink = (item, schemaProperty, linkProperty) => {
  const link = {
    meta: {
      relation: 'primary',
      // type: schemaProperty._flags._settings.__many || schemaProperty._flags._settings.__one,
      readOnly: false,
    },
    links: {
      self: `${responseHelper._baseUrl + item.type}/${
        item.id
      }/relationships/${linkProperty}`,
      related: `${responseHelper._baseUrl + item.type}/${
        item.id
      }/${linkProperty}`,
    },
    data: null,
  ...
  if (schemaProperty._flags._settings.__many) {
  ...
  if (schemaProperty._flags._settings.__one) {
  ...
  if (schemaProperty._flags._settings.__as) {
    const relatedResource = (schemaProperty._flags._settings.__one ||
      schemaProperty._flags._settings.__many)[0]
    ...
    link.links.self = `${
      responseHelper._baseUrl + relatedResource
    }/relationships/?${schemaProperty._flags._settings.__as}=${item.id}`
    // get full details of all linked resources
    // /rest/bookings/?filter[customer]=26aa8a92-2845-4e40-999f-1fa006ec8c63
    link.links.related = `${
      responseHelper._baseUrl + relatedResource
    }/?filter[${schemaProperty._flags._settings.__as}]=${item.id}`
    ...
    link.meta = {
      ...
      as: schemaProperty._flags._settings.__as,
      many: !!schemaProperty._flags._settings.__many,
      readOnly: true,
    }
  }
...
responseHelper._generateResponse = (
  request,
  resourceConfig,
  sanitisedData,
  handlerTotal
