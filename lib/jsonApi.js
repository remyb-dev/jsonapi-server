...
jsonApi._version = require(require('path').join(
  __dirname,
  '../package.json'
)).version
jsonApi._resources = {}
...
jsonApi.setConfig = apiConfig => {
  ...
  jsonApi._apiConfig.pathPrefix =
    apiConfig.urlPrefixAlias ||
    jsonApi._concatenateUrlPrefix(jsonApi._apiConfig)
...
jsonApi.define = resourceConfig => {
  if (!resourceConfig.resource.match(/^[A-Za-z0-9_]*$/)) {
    throw new Error(
      `Resource '${resourceConfig.resource}' contains illegal characters!`
    )
  }
  ...
  Object.keys(resourceConfig.attributes).forEach(attribute => {
    if (!attribute.match(/^[A-Za-z0-9_]*$/)) {
      throw new Error(
        `Attribute '${attribute}' on ${resourceConfig.resource} contains illegal characters!`
      )
    }
  })

  resourceConfig.searchParams = ourJoi.Joi.object(
    _.assign(
      {
        type: ourJoi.Joi.any()
          .required()
          .valid(resourceConfig.resource)
  ...
  resourceConfig.onCreate = _.pick.apply(
    ...
    [].concat(
      ...
      Object.keys(resourceConfig.attributes).filter(
        i =>
          resourceConfig.attributes[i]['$_terms'].metas.indexOf('readonly') ===
            -1 && !(resourceConfig.attributes[i]._settings || {}).__as
      )
    )
  )
}
