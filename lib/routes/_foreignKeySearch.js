    ...
    (request, resourceConfig, res) => {
      ...
      async.waterfall(
        [
          ...
          callback => {
            foreignKey = Object.keys(request.params)
              .filter(
                param =>
                  [
                    'include',
            ...
            request.params.relationships[foreignKey] =
              request.params[foreignKey]
            delete request.params[foreignKey]
            callback()
          },
          callback => {
            const foreignKeySchema = resourceConfig.attributes[foreignKey]
            if (!foreignKeySchema || !foreignKeySchema._flags._settings) {
            ...
            if (
              !(
                foreignKeySchema._flags._settings.__one ||
                foreignKeySchema._flags._settings.__many
              )
            ) {
              // eslint-disable-next-line standard/no-callback-literal
              return callback({
