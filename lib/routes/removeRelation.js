      ...
      async.waterfall(
        [
          ...
          (ourResource, callback) => {
            ...
            const isMany =
              resourceConfig.attributes[request.params.relation]._flags._settings
                .__many
            const isOne =
              resourceConfig.attributes[request.params.relation]._flags._settings.__one
            ...
            const keys = []
              .concat(theirResource[request.params.relation])
              .map(j => j.id)
          ...
          callback => {
            responseHelper._enforceSchemaOnObject(
              newResource,
              resourceConfig.attributes,
              callback
            )
          ...
          (sanitisedData, callback) => {
            ...
            response = responseHelper._generateResponse(
              request,
              resourceConfig,
              sanitisedData
            )
